    ...
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}">
        ...
        <div class="flex gap-2 items-center justify-between sm:hidden">
            ...
            @if ($paginator->onFirstPage())
                <span class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-gray-600 bg-gray-100 cursor-not-allowed leading-5 ">
                ...
                <span class="inline-flex rtl:flex-row-reverse gap-1">
                    ...
                    @if ($paginator->onFirstPage())
                        <span aria-disabled="true" aria-label="{{ __('pagination.previous') }}">
                            <span class="inline-flex items-center px-1.5 py-1.5 text-xs font-medium text-gray-500 bg-gray-100 cursor-not-allowed leading-5" aria-hidden="true">
                    ...
                    @foreach ($elements as $element)
                        ...
                        @if (is_string($element))
                            <span aria-disabled="true">
                                <span class="inline-flex items-center px-3 py-1.5 -ml-px text-xs font-medium text-gray-700 bg-gray-100 cursor-default leading-5">{{ $element }}</span>
                            </span>
                        @endif
                    ...
                    @else
                        <span aria-disabled="true" aria-label="{{ __('pagination.next') }}">
                            <span class="inline-flex items-center px-1.5 py-1.5 -ml-px text-xs font-medium text-gray-500 bg-gray-100 cursor-not-allowed leading-5" aria-hidden="true">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd" />
                                </svg>
                            </span>
