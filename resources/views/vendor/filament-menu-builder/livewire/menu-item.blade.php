<div class="item" data-id="{{ $item->id }}" wire:key="{{'menu-item-'.$item->id}}">
    <div @class([
        ...
        <div class="flex content-center items-center">
            <div class="border-r-2 border-gray-300 dark:border-gray-800 cursor-pointer bg-gray-400">
                <x-heroicon-o-arrows-up-down class="w-6 h-6 m-2 handle" />
            </div>
            <div class="ml-2 flex">
                <span class="font-medium">{{ json_decode($item->menu_name, true)[app()->getLocale()] }}</span>
                ...
                <x-filament::badge size="xs" class="ml-2 px-2" color="danger">
                    {{ $item->link_error ?? __('Link cannot be resolved!') }}
                </x-filament::badge>
                @endif
            </div>
        </div>
